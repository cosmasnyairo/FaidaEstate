...
$connect = new PDO("mysql:host=localhost;dbname=estate;charset=utf8mb4", "root", "");
...
{
 $query = "
 SELECT * FROM resident_details WHERE user_id = '$user_id' ORDER BY last_activity DESC LIMIT 1";
...
{
 $query = "SELECT username FROM resident WHERE user_id = '$user_id'";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 foreach($result as $row)
...
{
 $query = "SELECT is_type FROM resident_details WHERE user_id = '".$user_id."' ORDER BY last_activity DESC LIMIT 1"; 
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 $output = '';
...
{
 $query = "SELECT * FROM estate_forum WHERE to_user_id = '0' ORDER BY timestamp DESC";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 $output = '<ul class="list-unstyled">';
